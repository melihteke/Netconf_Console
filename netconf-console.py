...
if sys.hexversion < 0x02030000:
    print
    "This script needs python version >= 2.3"
    sys.exit(1)
...
def create_connection(host, port):
    ...
    if sock is None:
        print
        "Failed to connect to %s: %s" % (host, msg)
        sys.exit(1)
    return sock
...
if (o.wdefaults != "" and
        o.wdefaults not in ("trim", "explicit", "report-all", "report-all-tagged",
                            "true", "false")):
    print
    "Bad --with-defaults value: ", o.wdefaults
    sys.exit(1)
...
if o.proto == "ssh":
    try:
        ...
        print
        "You must install the python ssh implementation paramiko"
        print
        "in order to use ssh."
        sys.exit(1)
